...
class Admin(commands.Cog):
    ...
    @commands.command(help='Delete a game',
                      description="This will permanently delete the game (a backup will be taken" +
                      "first).")
    async def delete(self, ctx, name, confirmation_phrase=None):
        if (confirmation_phrase is not None and
            ...
            await ctx.send(
                f'Your complimentary backup can be downloaded using `!list-backups {name}`')
            ...
            await ctx.send(':warning: :warning: :warning: The game and associated discord channel' +
                           " will be permanently deleted. If you want to host it again you'll " +
                           " have to set that up manually - I cannot help you with that (although" +
                           " I will provide you with the backup). :warning: :warning: :warning:" +
                           "\nTo confirm the delete, use " +
                           f"`!delete {name} {self.confirmation_phrases[name]}`")
        else:
            self.confirmation_phrases[name] = random_string(10)
            await ctx.send(':no_entry_sign: Confirmation phrase did not match - to confirm the ' +
                           f'delete, use `!delete {name} {self.confirmation_phrases[name]}`')
