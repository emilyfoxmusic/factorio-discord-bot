...
class Roles(commands.Cog):
    ...
    @commands.command(help='Assign yourself the factorio role and get involved!',
                      name="count-me-in")
    async def count_me_in(self, ctx):
        ...
        await ctx.send(f'Yay, welcome {ctx.author.mention}. I\'ve messaged you ' +
                       'some more instructions :smiley:')
        await ctx.author.send("Hello! You should now be able to see any existing " +
                              "games under the 'factorio servers' category in discord " +
                              "(if any exist).\nI'm here to help you manage your games - " +
                              "I can create servers (with whatever mods you'd like), " +
                              "start/stop your games, take backups, and I'll even automatically " +
                              "stop servers when no one's using them! Use `!help` for details on " +
                              "the commands I understand. (I can do this in DM for your " +
                              "convenience.) Enjoy factorio-ing and let me know if there's " +
                              "anything I can help with! :partying_face:")

    @commands.command(help='Removes the factorio role so you won\'t see the games',
                      name="count-me-out")
    async def count_me_out(self, ctx):
        ...
        await ctx.send('Okay, come back soon! :cry:')
...
def get_bot_role(guild, bot):
    bot_role = list(filter(lambda role: not role.is_default(),
                    guild.get_member(bot.user.id).roles))
    if len(bot_role) > 1:
